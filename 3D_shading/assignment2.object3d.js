...
class Object3D
{
    ...
    createVAO( gl, shader )
    {
        ...
        // Each vertex has position (vec3) and normal (vec3), so stride is 6 floats
        const stride = 6 * Float32Array.BYTES_PER_ELEMENT;
    
        // Set up position attribute (first 3 floats in each vertex)
        const positionAttribLocation = shader.getAttributeLocation('a_position');
        ...
        // Set up normal attribute (next 3 floats in each vertex)
        const normalAttribLocation = shader.getAttributeLocation('a_normal');
        if (normalAttribLocation !== -1) {  // Check if attribute exists
...
class ShadedObject3D extends Object3D { 
    ...
    render( gl )
    {
        ...
        // Use the correct property names (kA, kD, kS) and create Float32Arrays properly
        this.shader.setUniform3f('u_material.kA', new Float32Array([
            this.material.kA[0],
            this.material.kA[1],
            this.material.kA[2]
        ]));
        
        this.shader.setUniform3f('u_material.kD', new Float32Array([
            this.material.kD[0],
            this.material.kD[1],
            this.material.kD[2]
        ]));
        
        this.shader.setUniform3f('u_material.kS', new Float32Array([
            this.material.kS[0],
            this.material.kS[1],
            this.material.kS[2]
        ]));
